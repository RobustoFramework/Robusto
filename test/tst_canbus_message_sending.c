...
void canbus_init_defs() {
    ...
    remote_peer->canbus_address = CONFIG_ROB_NETWORK_TEST_CANBUS_CALL_ADDR;
    remote_peer->supported_media_types = robusto_mt_canbus;
...
void tst_canbus_message_send_message_sync(void)
{
   ...
   // TODO: Fix above and enable content of send message media type.
    ...
    while ((r_millis() < startime + 10000)) //&& (retval != ROB_OK)) 
    {
        */
        ...
        retval = send_message_raw_internal(remote_peer, robusto_mt_canbus, tst_strings_msg, tst_strings_length, NULL, true, false, 0, robusto_mt_none);
    /*
        attempts++;
    }
    ROB_LOGI("TEST", "Send attempts: %i, time: %lu ", attempts, startime - r_millis());  
    */
    //rob_ret_val_t retval = robusto_send_message_media_type(remote_peer, tst_strings_res, tst_strings_length, robusto_mt_canbus, false);
...
void tst_canbus_message_send_presentation(char * dest) {

	r_delay(1000);
	// Send it.
	add_peer_by_canbus_address(dest, CONFIG_ROB_NETWORK_TEST_CANBUS_CALL_ADDR);
}


void tst_canbus_message_send_message(void)
{
    
	r_delay(4000);

	ROB_LOGI("TEST", "In tst_canbus_message_send_message.");
