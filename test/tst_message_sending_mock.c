...
void init_defs_mock()
{
    if (robusto_peers_find_peer_by_name("TEST_MOCK_1"))
    {
        return;
    }
    robusto_peer_t * test_peer_mock = robusto_add_init_new_peer("TEST_MOCK_1", (rob_mac_address *)kconfig_mac_to_6_bytes(11), robusto_mt_mock);
    ROB_LOGI("TEST", "ADDED TEST_MOCK_1");
...
//    set_message_expectation(MMI_STRINGS); 
    ...
    robusto_peer_t * test_peer_mock = robusto_peers_find_peer_by_name("TEST_MOCK_1");
    ...
    if (!robusto_waitfor_queue_state(q_state, 1500, &ret_val))
    {
