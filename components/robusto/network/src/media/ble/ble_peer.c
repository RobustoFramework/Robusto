...
/**
 * @brief Check if there are any Robusto peers with the same BLE address.
 * 
 ...
 */
struct robusto_peer *
ble_peer_find_robusto_peer_by_ble_addr(rob_mac_address *mac_address)
{
...
void ble_peer_init_peer(robusto_peer_t *r_peer) {
    // The connection handle is unset
    r_peer->ble_conn_handle = -1;
    ble_spp_client_scan();
}
